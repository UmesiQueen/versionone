...
const BookingForm = ({
    className,
    calendlyUrl = "https://calendly.com/aguhenrychuks/30min",
    ...
    const onSubmit: SubmitHandler<BookingFormData> = async (data) => {
        try {
            ...
            if (response.ok) {
                ...
                const fullCalendlyUrl = `${calendlyUrl}?${params.toString()}`

                if (onSuccess) {
                    onSuccess(data)
                } else {
                    setModal({ open: true, name: data.fullName, calendlyUrl: fullCalendlyUrl })
                }
            } else {
